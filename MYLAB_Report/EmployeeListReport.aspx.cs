...
namespace MYLAB_Report
{
    public partial class EmployeeListReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    GetEmployeeListReport(Id, FDate, TDate);
                }
        ...
        private void GetEmployeeListReport(string Id, string FDate, string TDate)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    SqlCommand sqlcmd = new SqlCommand("PRC_RP_GETEMPLOYEELIST", sqlcon);
                    ...
                    try
                    {
                        CrystalReport.Load(Server.MapPath("~/Reports/EmployeeList.rpt"));
                    }
                    catch
                    {
                        Label.Enabled = true;
                    ...
                    CrystalReport.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, "EmployeeList-" + DateTime.Now);
                    Response.End();
                }
                catch (Exception)
                {
